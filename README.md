...
**03:** Table name: `like`, columns: `id1`, `id2`
- `id1` is a foreign key reference to a learner
- `id2` is a foreign key reference to a learner
...
**04:** SQL query to load the CSV file `data/learners.csv` for the `learner` table
<br>Write the SQL in the file `sql/04-load-table-learner.sql`
...
**06:** SQL query to load the CSV file `data/likes.csv` for the `likes` table
...
**07:** Find the names of all learners who are friends with someone named Gabriel.
<br>Write the SQL in the file `sql/07-query-friends-gabriel.sql`

**08:** For every learner who likes someone 2 or more grades younger than themselves, return that learner's name and grade, and the name and grade of the learner they like.
...
**11:** For every situation where student A likes student B, but we have no information about whom B likes (that is, B does not appear as an ID1 in the like table), return A and B's names and grades.
